...
func _physics_process(delta: float) -> void:
	...
	if has_item_ready:
		...
		if new_item_node_array.size() == 3:
			...
			if next_component.has_space and next_component_2.has_space and next_component_3.has_space:
				move_item(direction, next_component, new_item_node_array[0])
				move_item(get_direction(1), next_component_2, new_item_node_array[1])
...
# could add all or most to process class.
func item_arrived():
...
func cut_item():
	...
	var output_array: Array[int] = Globals.get_cutter_recipe(item_storage.pop_back())
...
func _on_process_timer_timeout() -> void:
	if has_storage() and !has_item_ready:
		cut_item()
		process_timer.start()
